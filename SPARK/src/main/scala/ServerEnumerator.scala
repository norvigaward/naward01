 ...
 *
 * the aggregated domain count here does not add up to the total number of domains,
 * as a server can have things in both the Server header and the other headers
 */
object ServerEnumerator {
  def main(args : Array[String]): Unit = {
    ...
    val exportHdfsPath = args(1) + "/enum_" + System.currentTimeMillis() // "/user/naward01"
